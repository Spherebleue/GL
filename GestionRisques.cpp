            ...
            for (int j = 0; j < int(Empreinte::format.size()); j++)
            {
                ...
                if(attribut.compare("")!=0)
                {
                    if (attribut.compare("1") == 0)
                    {
                        ...
                        if (dynamic_cast <Numerique*>(vectEmpreinte[i]->getListeAttribut()[j])->getValeur() <= (moyenne + 2*ecartType)
                        && dynamic_cast <Numerique*>(vectEmpreinte[i]->getListeAttribut()[j])->getValeur() >= (moyenne - 2*ecartType) )
                            compteurSymptomes ++;
                        }
                    }
                }
